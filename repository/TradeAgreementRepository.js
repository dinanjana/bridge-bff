...
const getAll = async () => {
    ...
    const url = ENDPOINTS.TRADE_AGREEMENT.base + '/' + client + ENDPOINTS.TRADE_AGREEMENT.context;
    try {
...
const get = async (id) => {
    const url = ENDPOINTS.TRADE_AGREEMENT.base + '/' + client + ENDPOINTS.TRADE_AGREEMENT.context + '/' + id;
    try {
        log.info(`Calling ${url}`);
        const response = await axios.get(url);
        return response;
