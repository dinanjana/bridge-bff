...
const getAll = async () => {
    ...
    const url = ENDPOINTS.MATERIALTYPE.base + '/' + client + ENDPOINTS.MATERIALTYPE.context;
    try {
        ...
        clog.error(` Error MaterialType code- ${err}`, err);
        throw err;
    }
};

const get = async (id) => {
    const url = ENDPOINTS.MATERIALTYPE.base + '/' + client + ENDPOINTS.MATERIALTYPE.context + '/' + id;
    try {
        log.info(`Calling ${url}`);
        const response = await axios.get(url);
        return {status : response.status, data :  response.data};
...
const post = async (body) => {
    ...
    try {
        ...
        log.error(` Error MaterialType code- ${err}`, err);
        throw err;
    }
};
